...
// ----------------------------------------------------------------------------
struct Mac final {
	static constexpr int SIZE = 6; // mac_ 주소는 6바이트니까.

	// 복사생성자constructor
	Mac() {} 
	Mac(const Mac& r) { memcpy(this->mac_, r.mac_, SIZE); } //객체주소 복사해서 new 객체 생성
	Mac(const uint8_t* r) { memcpy(this->mac_, r, SIZE); } //배열 복사해서 new 객체 생성
	Mac(const std::string& r); 

	// assign operator//대입 생성자
